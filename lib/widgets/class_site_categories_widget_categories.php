...
class Bcat_WidgetCategories extends WP_Widget {
	...
	function __construct () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Zeigt eine Liste aller Seiten-Kategorien an.', SITE_CATEGORIES_I18N_DOMAIN));
		parent::__construct(__CLASS__, __('Seiten-Kategorien', SITE_CATEGORIES_I18N_DOMAIN), $widget_ops);
	}
	...
	function form($instance) {
		...
		$defaults = array( 
			...
			'open_blank'			=>	0,
			'show_more_link'		=>	1,
			'landing_link_label'	=>	__('mehr Kategorien', SITE_CATEGORIES_I18N_DOMAIN)
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id('title') ?>"><?php _e('Titel:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_style') ?>"><?php _e('Darstellen als:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			<select id="<?php echo $this->get_field_id( 'show_style' ); ?>" 
				...
				<option value="ol" <?php if ($instance['show_style'] == "ol") { echo ' selected="selected" '; } ?>><?php 
					_e('Geordnete Liste (ol)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="ol-nested" <?php if ($instance['show_style'] == "ol-nested") { echo ' selected="selected" '; } ?>><?php 
					_e('Geordnete Liste verschachtelt (ol)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="ul" <?php if ($instance['show_style'] == "ul") { echo ' selected="selected" '; } ?>><?php 
					_e('Unogeordnete Liste (ul)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="ul-nested" <?php if ($instance['show_style'] == "ul-nested") { echo ' selected="selected" '; } ?>><?php 
					_e('Unogeordnete Liste verschachtelt (ul)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="select-nested" <?php if ($instance['show_style'] == "select-nested") { echo ' selected="selected" '; } ?>><?php 
					_e('Dropdown verschachtelt (auswählen)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="select-flat" <?php if ($instance['show_style'] == "select-flat") { echo ' selected="selected" '; } ?>><?php 
					_e('Dropdown (auswählen)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Seiten-Kategorie Übergeordnet:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<?php
				...
				$bcat_args = array(
					...
					'hide_empty'		=>	false,
					'show_count'		=>	true,
					'show_option_none'	=>	__('Alle', SITE_CATEGORIES_I18N_DOMAIN), 
		...
		<p>
			<label for="<?php echo $this->get_field_id('category_filter') ?>"><?php _e('Seiten-Kategorien Einschließen/Ausschließen:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<select id="<?php echo $this->get_field_id( 'category_filter' ); ?>" 
				...
				<option value="" <?php if ($instance['category_filter'] == "") { echo ' selected="selected" '; }?>><?php 
					_e('Zeige alles', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="include" <?php if ($instance['category_filter'] == "include") { echo ' selected="selected" '; }?>><?php 
					_e('Einschließen', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="exclude" <?php if ($instance['category_filter'] == "exclude") { echo ' selected="selected" '; }?>><?php 
					_e('Ausschließen', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="exclude_tree" <?php if ($instance['category_filter'] == "exclude_tree") { echo ' selected="selected" '; }?>><?php 
					_e('Baum ausschließen', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
			</select><br />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('category_ids') ?>"><?php _e('IDs für Seiten-Kategorien (durch Kommas getrennt):', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'per_page' ); ?>"><?php 
				_e('Anzahl der anzuzeigenden Seiten-Kategorien (0 für alle):', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('ordering') ?>"><?php _e('Sortierung:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<select id="<?php echo $this->get_field_id( 'ordering' ); ?>" 
				...
				<option value="id" <?php if ($instance['ordering'] == "id") { echo ' selected="selected" '; }?>><?php 
					_e('Kategorie ID', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
			</select>
		...
		<p>
			<label for="<?php echo $this->get_field_id('hide_empty') ?>"><?php _e('Leere Seiten-Kategorien ausblenden:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<input type="radio" name="<?php echo $this->get_field_name( 'hide_empty'); ?>" id="<?php echo $this->get_field_id('hide_empty') ?>_yes" 
				value="1" <?php if ($instance['hide_empty'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('hide_empty') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			
			<input type="radio" name="<?php echo $this->get_field_name( 'hide_empty'); ?>" id="<?php echo $this->get_field_id('hide_empty') ?>_no" 
				value="0" <?php if ($instance['hide_empty'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('hide_empty') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('show_counts') ?>"><?php _e('Anzahl der Seiten-Kategorien anzeigen:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<input type="radio" name="<?php echo $this->get_field_name( 'show_counts'); ?>" id="<?php echo $this->get_field_id('show_counts') ?>_yes" 
				value="1" <?php if ($instance['show_counts'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('show_counts') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			
			<input type="radio" name="<?php echo $this->get_field_name( 'show_counts'); ?>" id="<?php echo $this->get_field_id('show_counts') ?>_no" 
				value="0" <?php if ($instance['show_counts'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('show_counts') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		</p>


		<p>
			<label for="<?php echo $this->get_field_id('icon_show') ?>"><?php _e('Symbole für Seiten-Kategorie anzeigen: (nur Listen)', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<input type="radio" name="<?php echo $this->get_field_name( 'icon_show'); ?>" id="<?php echo $this->get_field_id('icon_show') ?>_yes" 
				value="1" <?php if ($instance['icon_show'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('icon_show') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			
			<input type="radio" name="<?php echo $this->get_field_name( 'icon_show'); ?>" id="<?php echo $this->get_field_id('icon_show') ?>_no" 
				value="0" <?php if ($instance['icon_show'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('icon_show') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('icon_size') ?>"><?php _e('Symbolgröße:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			<input type="text" name="<?php echo $this->get_field_name('icon_size'); ?>" id="<?php echo $this->get_field_id('icon_size'); ?>" 
				class="" size="5" value="<?php echo $instance['icon_size'] ?>"/>px  <?php _e('Quadrat', SITE_CATEGORIES_I18N_DOMAIN); ?>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('open_blank') ?>"><?php _e('Links in neuem Fenster öffnen:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<input type="radio" name="<?php echo $this->get_field_name( 'open_blank'); ?>" id="<?php echo $this->get_field_id('open_blank') ?>_yes" 
				value="1" <?php if ($instance['open_blank'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('open_blank') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			
			<input type="radio" name="<?php echo $this->get_field_name( 'open_blank'); ?>" id="<?php echo $this->get_field_id('open_blank') ?>_no" 
				value="0" <?php if ($instance['open_blank'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('open_blank') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('show_more_link') ?>"><?php _e('Landeseite Link unter der Kategorieliste:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<input type="radio" name="<?php echo $this->get_field_name( 'show_more_link'); ?>" id="<?php echo $this->get_field_id('show_more_link') ?>_yes" 
				value="1" <?php if ($instance['show_more_link'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('show_more_link') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			
			<input type="radio" name="<?php echo $this->get_field_name( 'show_more_link'); ?>" id="<?php echo $this->get_field_id('show_more_link') ?>_no" 
				value="0" <?php if ($instance['show_more_link'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('show_more_link') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'landing_link_label' ); ?>"><?php 
				_e('Etikett für Link:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			...
			if (($categories) && (count($categories))) {
				...
				if (intval($instance['show_more_link'])) {
					...
					if (isset($instance['landing_link_label']))
						...
						$data['landing']['link_label'] = __('Weitere Kategorien', SITE_CATEGORIES_I18N_DOMAIN);								
				}			
			}		
			restore_current_blog();
...
function process_categories_widget_list_display($content, $data, $args) {
	...
	if ((isset($data['categories'])) && (count($data['categories']))) {
		...
		if (($args['show_style'] == "ol") || ($args['show_style'] == "ol-nested")) { $content .= '<ol class="site-categories site-categories-widget">'; }
			...
			$content .= '<option value="">'. __('Kategorie wählen', SITE_CATEGORIES_I18N_DOMAIN) .'</option>';
