...
class Bcat_WidgetCategorySites extends WP_Widget {
	...
	function __construct () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Zeigt eine Liste aller Seiten an, die zur ausgewählten Seiten-Kategorie gehören.', SITE_CATEGORIES_I18N_DOMAIN));
		parent::__construct(__CLASS__, __('Seiten-Kategorien Blogs', SITE_CATEGORIES_I18N_DOMAIN), $widget_ops);
	}
	...
	function form($instance) {
		...
		$defaults	=	array(
			...
			'open_blank'			=>	0,
			'show_more_link'		=>	1,
			'landing_link_label'	=>	__('weitere Webseiten', SITE_CATEGORIES_I18N_DOMAIN)	
		);
		...
		<p>
			<label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Seiten-Kategorien:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			<?php
				...
				$bcat_args = array(
					...
					'hide_empty'		=>	false,
					'show_count'		=>	true,
					'show_option_none'	=>	__('Zeige alle', SITE_CATEGORIES_I18N_DOMAIN), 
			...
			?><input type="checkbox" id="<?php echo $this->get_field_id( 'include_children' ); ?>" <?php if ($instance['include_children'] == "on") {
				echo ' checked="checked" '; } ?> name="<?php echo $this->get_field_name( 'include_children'); ?>"  /> <label for="<?php echo $this->get_field_id( 'include_children' ); ?>"><?php _e('Untergeordnete Kategorien einschließen? Wenn nicht festgelegt, werden nur Kategorien der obersten Ebene eingeschlossen', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		</p>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id( 'show_style' ); ?>" 
				...
				<option value="ul" <?php if ($instance['show_style'] == "ul") { echo ' selected="selected" '; }?>><?php 
					_e('Ungeordnete Liste (ul)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
				<option value="select" <?php if ($instance['show_style'] == "select") { echo ' selected="selected" '; }?>><?php _e('Dropdown (auswählen)', SITE_CATEGORIES_I18N_DOMAIN); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('category_ids') ?>"><?php _e('Seiten-Kategorie-IDs (durch Kommas getrennt):', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('blog_filter') ?>"><?php _e('Blogs einschließen/ausschließen:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id('blog_ids') ?>"><?php _e('Blog IDs (durch Kommas getrennt):', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'per_page' ); ?>"><?php 
				_e('Anzahl der anzuzeigenden Seiten (0 für alle):', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		if (function_exists('get_blog_avatar')) { 
			?>
			<p>
				<label for="<?php echo $this->get_field_id('icon_show') ?>"><?php _e('Blog-Symbole anzeigen:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
				<input type="radio" name="<?php echo $this->get_field_name( 'icon_show'); ?>" id="<?php echo $this->get_field_id('icon_show') ?>_yes" 
					value="1" <?php if ($instance['icon_show'] == "1") { echo ' checked="checked" '; } ?> /> <label for="<?php 
						echo $this->get_field_id('icon_show') ?>_yes"><?php _e('Ja', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
				...
				<input type="radio" name="<?php echo $this->get_field_name( 'icon_show'); ?>" id="<?php echo $this->get_field_id('icon_show') ?>_no" 
					value="0" <?php if ($instance['icon_show'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php 
						echo $this->get_field_id('icon_show') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
			</p>
			...
			_e('<p>Blog Avatar erforderlich <a target="_blank" href="https://n3rds.work/wiki/piestingtal-source-wiki/avatare-plugin/">Avatare</a> Plugin.</p>', SITE_CATEGORIES_I18N_DOMAIN);
		}
		?>
		<p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_more_link') ?>"><?php _e('Landeseiten-Link unten Liste der Webseiten:', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			...
			<input type="radio" name="<?php echo $this->get_field_name( 'show_more_link'); ?>" id="<?php echo $this->get_field_id('show_more_link') ?>_no" 
				value="0" <?php if ($instance['show_more_link'] == "0") { echo ' checked="checked" '; } ?> /> <label for="<?php echo $this->get_field_id('show_more_link') ?>_no"><?php _e('Nein', SITE_CATEGORIES_I18N_DOMAIN); ?></label><br />
			
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'landing_link_label' ); ?>"><?php 
				_e('Beschriftung für Link:', SITE_CATEGORIES_I18N_DOMAIN); ?></label>
		...
		if (!$data) {
			...
			if (intval($instance['show_more_link'])) {
				...
				if (isset($instance['landing_link_label']))
					...
					$data['landing']['link_label'] = __('Weitere Webseiten', SITE_CATEGORIES_I18N_DOMAIN);								
			}
...
function process_categories_widget_list_sites_display($content, $data, $args) {
	...
	if (($data['sites']) && (count($data['sites']))) { 
		...
		if ($args['show_style'] == "ol") { $content .= '<ol class="site-categories site-categories-widget">'; }
			...
			$content .= '<option value="">'. __('Wähle Webseite', SITE_CATEGORIES_I18N_DOMAIN) .'</option>';
